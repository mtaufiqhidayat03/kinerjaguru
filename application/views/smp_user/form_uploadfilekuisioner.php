...
<h5 class="modal-title">Upload File Penilaian Kuisioner</h5>
...
<tr valign="top">
   <td height="50"></td>
    ...
    <input name="editid_guru" id="editid_guru" type="text" readonly value="<?php foreach($n1 as $baris3) { echo $baris3->nuptk; } ?>" />
	</td>
	<td></td>
</tr>
  <?php } ?>
	...
	$(document).on('click', "button#submit_file", function(){
		...
		if ($('form.form_upload_file').valid() && document.querySelector("#pdf-file").value !== "") {
			...
			cache: false,
      		contentType: false,
      		processData: false,
			url: "<?php echo base_url().FOLDER_SMP_USER;?>kuisioneruser/aksiuploadfilekuisioner",
			data: databaru,
				beforeSend: function(){
				},
