...
<tr valign=top>
	...
	<input name="npsn_nss" readonly type="text" required class="form-control" id="npsn_nss" placeholder="Silahkan masukkan NPSN/NSS Sekolah" value="<?php foreach($n2 as $baris) { echo $baris->npsn_nss_smp; }?>"></div></div>
	</td>
  </tr>
  <tr valign=top>
    <td width="6" height="60"></td>
...
$(function() {
	$(document).on('click', "button#submit_data", function(){
		...
		if ($('form.form_adddata').valid()) {
            ...
            contentType: false,
            processData: false,
			url: "<?php echo base_url().FOLDER_SMP_USER;?>guru/aksiaddguru",
			data: databaru,
				beforeSend: function(){
				},
