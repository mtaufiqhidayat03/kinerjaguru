...
{
	$(function(){
		if($('#id_kompetensi_indikator_smp').hasClass("select2-hidden-accessible")) {
		$('#id_kompetensi_indikator_smp').val(null).trigger('change');
		} 
		var opt = $('#kelompok_kompetensi').find(':selected');
		var idkelompok = opt.val();
        $('#id_kompetensi_indikator_smp, #id_kompetensi_indikator_smp_validate').select2({
           allowClear: true,
		   language: "id",
...
<tbody>
  ...
  <tr valign=top>
	...
	<div class="kt-input-icon kt-input-icon--left">
	<span class="kt-input-icon__icon kt-input-icon__icon--left"><span><i class="la la-keyboard-o"></i></span></span>
    <select name="id_kompetensi_indikator_smp" data-rel='chosen' class="form-control placeholder" id="id_kompetensi_indikator_smp" required>
...
$(function() {
	$(document).on('click', "button#submit_data", function(){
		...
		if ($('form.form_adddata').valid()) {
            ...
            contentType: false,
            processData: false,
			url: "<?php echo base_url().FOLDER_SMP_USER;?>indikator/aksiaddindikator",
			data: databaru,
				beforeSend: function(){
				},
