...
<tbody>
  ...
  <tr valign=top>
    <td height="60"></td>
    <td>
    <label>Nama Kuisioner</label></td>
    <td>:</td>
    <td>
    <div class="form-group row">
	<div class="kt-input-icon kt-input-icon--left">
	<span class="kt-input-icon__icon kt-input-icon__icon--left"><span><i class="la la-keyboard-o"></i></span></span>
    <input class="form-control" autofocus name="nama_kuisioner" type="text"  size="40" id="nama_kuisioner" required autofocus placeholder="Silahkan masukkan nama kuisioner" />
    </div></div>
	</td>
	<td></td>
  </tr>
...
$(function() {
	$(document).on('click', "button#submit_data", function(){
		...
		if ($('form.form_adddata').valid()) {
            ...
            contentType: false,
            processData: false,
			url: "<?php echo base_url().FOLDER_SMP;?>kuisioner/aksiaddkuisioner",
			data: databaru,
				beforeSend: function(){
				},
