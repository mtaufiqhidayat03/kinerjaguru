...
{
	$(function(){
		//$('#editid_kompetensi_indikator_sd').select2('destroy');
		if($('#editid_kompetensi_indikator_sd').hasClass("select2-hidden-accessible")) {
		$('#editid_kompetensi_indikator_sd').val(null).trigger('change');
		} 
...
<?php foreach($n2 as $baris) { ?>
  ...
  <tr valign=top>
	...
	<div class="kt-input-icon kt-input-icon--left">
	<span class="kt-input-icon__icon kt-input-icon__icon--left"><span><i class="la la-keyboard-o"></i></span></span>
    <select name="editid_kompetensi_indikator_sd" data-rel='chosen' class="form-control" id="editid_kompetensi_indikator_sd" required >
	<option value=""></option>
    </select>
	</div>
	</div> 
  ...
  <tr></td>
    ...
    <input name="editkelompok_kompetensi2" id="editkelompok_kompetensi2" type="text" readonly value="<?php echo $baris->id_kelompok_kompetensi_sd; ?>" />
	<input name="editid_kompetensi_indikator_sd2" id="editid_kompetensi_indikator_sd2" type="text" readonly value="<?php echo $baris->id_kompetensi_indikator_sd; ?>" />
	</td>
	<td></td>
</tr>
  <?php } ?>
...
$(function() {
	$(document).on('click', "button#edit_data", function(){
		...
		if ($('form.form_editdata').valid()) {
            ...
            contentType: false,
            processData: false,
			url: "<?php echo base_url().FOLDER_SD;?>indikator/aksieditindikator",
			data: databaru,
				beforeSend: function(){
				},
