...
<button type="button" class="btn btn-info btn-elevate2 btn-elevate-air2" id="submit_jenis_guru"><i class="fa fa-pen"></i> Pilih Jenis Guru</button>
	...
	$(document).on('click', "button#submit_jenis_guru", function(){
		...
		if ($('form.form_jenis_guru_sekolah').valid()) {
			...
			cache: false,
      		contentType: false,
      		processData: false,
			url: "<?php echo base_url().FOLDER_SMP;?>guru/aksieditgurusekolahmapel",
			data: databaru,
				beforeSend: function(){
				},
			   ...
			   if (data != "") {
				...
				"hideMethod": "slideUp"
				 ...
				 unblockPageUI();
					...
					$('#guru_sekolahmapel').modal('hide');
					$('#guru_sekolahmapel').on('hidden.bs.modal', function(){
