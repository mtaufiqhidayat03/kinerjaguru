...
class M_guruuser extends CI_Model {
	...
	function viewguru() {
        $sql="SELECT * FROM `".M_GURU_SMP."`";
        $query=$this->db->query($sql);
        return $query->result();
	}

	function viewguru2($npsn_nss) {
        $sql="SELECT * FROM `".M_GURU_SMP."` as a left join `".D_GURU_SMP.$this->session->userdata('tahun')."` as b  ON a.nuptk=b.nuptk_guru_smp where npsn_nss_guru_smp=?";
	...
	function updategurusekolah2($data_guru2) {
		$this->db->insert("`".D_GURU_SMP.$this->session->userdata('tahun')."`", $data_guru2);
	}
   ...
   // if (isset($npsn_nss) and $npsn_nss != "") {
	   $sWhere = "where npsn_nss_guru_smp='".$npsn_nss."' ";
	//}
	...
	$sQuery = "SELECT COUNT(".$sIndexColumn.") as 'Count' FROM  $sTable where npsn_nss_guru_smp='".$npsn_nss."'";
