...
class M_cetakkinerja extends CI_Model {
	...
	function getdatagurukompetensi($nuptk) {
		$sql="SELECT * FROM `".M_KOMPETENSI_SMP."` as b left join `".M_KELOMPOK_KOMPETENSI_SMP."` as c ON b.id_kelompok_kompetensi_smp=c.id_kelompok left join `".D_GURU_SMP.$_SESSION["tahun"]."` as a ON c.hub_jenis_guru=a.jenis_guru where a.nuptk_guru_smp=?";
		$query=$this->db->query($sql,array($nuptk));
		return $query->result();
	}
