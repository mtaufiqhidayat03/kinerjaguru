<?php
class M_indikatoradmin extends CI_Model {

	function addindikator($data_indikator) {
		$this->db->insert(M_INDIKATOR_SMP,$data_indikator);
	}

	function namaindikator($id_kompetensi) {
        $sql="SELECT * FROM `".M_KOMPETENSI_SMP."` where id_kompetensi=?";
        $query=$this->db->query($sql,array($id_kompetensi));
        return $query->result();
	}
	
	function getdataindikator($id_indikator) {
		$sql="SELECT * FROM `".M_INDIKATOR_SMP."` as a left join `".M_KOMPETENSI_SMP."` as b ON a.id_kompetensi_indikator_smp=b.id_kompetensi where a.id_indikator=?";
	...
	function updateindikator($data_indikator,$id_indikator) {
        ...
        $this->db->update(M_INDIKATOR_SMP,$data_indikator);
	}
	
    function deleteindikator($id_indikator){
        $this->db->where('id_indikator', $id_indikator);
        $this->db->delete(M_INDIKATOR_SMP);
	}

	function datakompetensi($search, $page, $id_kelompok){
		$this->db->where('id_kelompok_kompetensi_smp', $id_kelompok);
		...
		$query2=$this->db->get(M_KOMPETENSI_SMP);
	...
	function indikator_list($id_kompetensi) {
		...
		$sTable = "`".M_INDIKATOR_SMP."` as a left join `".M_KOMPETENSI_SMP."` as b ON a.id_kompetensi_indikator_smp=b.id_kompetensi left join `".M_KELOMPOK_KOMPETENSI_SMP."` as c ON b.id_kelompok_kompetensi_smp=c.id_kelompok" ;
		$sLimit = "";
		/*  Paging */
		...
		if (isset($id_kompetensi) and $id_kompetensi != "") {
			$sWhere = "where a.id_kompetensi_indikator_smp='".$id_kompetensi."' ";
		}
