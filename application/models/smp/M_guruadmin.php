...
class M_guruadmin extends CI_Model {
	...
	function viewguru() {
        $sql="SELECT * FROM `".M_GURU_SMP."` as a left join `".D_GURU_SMP.$this->session->userdata('tahun')."` as b  ON a.nuptk=b.nuptk_guru_smp left join `".M_SMP."` as c ON c.npsn_nss=b.npsn_nss_guru_smp";
        $query=$this->db->query($sql);
        return $query->result();
	}

	function viewguru2($npsn_nss) {
        $sql="SELECT * FROM `".M_GURU_SMP."` as a left join `".D_GURU_SMP.$this->session->userdata('tahun')."` as b  ON a.nuptk=b.nuptk_guru_smp left join `".M_SMP."` as c ON c.npsn_nss=b.npsn_nss_guru_smp where npsn_nss_guru_smp=?";
        $query=$this->db->query($sql, array($npsn_nss));
        return $query->result();
	}

	function nama_sekolah($npsn_nss) {
        $sql="SELECT npsn_nss,nama_sekolah FROM `".M_SMP."` where npsn_nss=?";
		$query=$this->db->query($sql, array($npsn_nss));
        return $query->result();
	}
	...
	function getdataguru2($nuptk){
        $sql="SELECT * FROM `".M_SMP."` as a left join `".D_GURU_SMP.$_SESSION["tahun"]."` as b  ON a.npsn_nss=b.npsn_nss_guru_smp left join `".M_GURU_SMP."` as c ON b.nuptk_guru_smp=c.nuptk where nuptk_guru_smp=?";
        $query=$this->db->query($sql,array($nuptk));
        return $query->result();
	}

    function getsekolah() {
        $sql="SELECT npsn_nss,nama_sekolah FROM `".M_SMP."`";
        $query=$this->db->query($sql);
        return $query->result();
	}
	
    function datasekolahsmp($search, $page){
    ...
    function deleteguru($nuptk){
        $this->db->where('nuptk', $nuptk);
		$this->db->delete(M_GURU_SMP);
		$this->db->where('username', $nuptk);
		$this->db->delete(M_USERS);
	}
	
	function deleteguru2($nuptk){
		$this->db->where('nuptk_guru_smp', $nuptk);
        $this->db->delete("`".D_GURU_SMP.$this->session->userdata('tahun')."`");
    }
    
    function addguru($data_guru) {
        $this->db->insert(M_GURU_SMP,$data_guru);
	}
    ...
    function updategurusekolah($data_guru) {
		$this->db->insert("`".D_GURU_SMP.$this->session->userdata('tahun')."`", $data_guru);
	}
	...
	function deletejenisgurumapel($nuptk){
		...
		$this->db->where('nuptk_guru_smp', $nuptk);
		$this->db->update("`".D_GURU_SMP.$this->session->userdata('tahun')."`");
	}

	function updategurusekolahmapel($data_guru,$nuptk) {
		$this->db->where('nuptk_guru_smp', $nuptk);
		$this->db->update("`".D_GURU_SMP.$this->session->userdata('tahun')."`",$data_guru);
	}
    ...
    'CONCAT(floor(datediff(CURDATE(), tmt_guru)/365),\' tahun \',floor((datediff(CURDATE(), tmt_guru)-(floor(datediff(CURDATE(), tmt_guru)/365)*365))/30),\' bulan \', (datediff(CURDATE(), tmt_guru) mod 30),\' hari\')','jenis_kelamin','pendidikan_terakhir','program_keahlian','jenis_guru','detail_guru','npsn_nss','npsn_nss_guru_smp');
	$sIndexColumn = "a.nuptk";
	$sTable = "`".M_GURU_SMP."` as a left join `".D_GURU_SMP.$_SESSION["tahun"]."` as b ON a.nuptk=b.nuptk_guru_smp left join `".M_SMP."` as c ON c.npsn_nss=b.npsn_nss_guru_smp ";
	$sLimit = "";
	/*  Paging */
    ...
    if (isset($npsn_nss) and $npsn_nss != "") {
	   $sWhere = "where b.npsn_nss_guru_smp='".$npsn_nss."' ";
	}

	if (!empty($params['search']['value']))
	{
		if (isset($npsn_nss) and $npsn_nss != "") {
            $sWhere = "where b.npsn_nss_guru_smp='".$npsn_nss."' and (";
        } else {
            $sWhere = "where (";
        }
			...
			if ( $i == 1)
			{
				if ($aRow[$aColumns[17]] == "") {
					...
					if ($aRow[$aColumns[14]] == "") {
						$row[] = 
						"<div class='btn-group-vertical' role='group'>
						<a data-toggle='modal' href='guru/form_gurumapel?nuptk=".$aRow['nuptk']."&npsn_nss=".$aRow['npsn_nss']."' data-target='#guru_sekolahmapel' class='btn btn-success btn-sm btnku btn-elevate btn-elevate-air' id='guru-sekolahmapel' data-id='".$aRow['nuptk']."'><i class='flaticon-interface-5'></i> Pilih Menjadi Guru<br/>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Mata Pelajaran/Kelas</a>
						<a data-toggle='modal' href='guru/form_editguru?nuptk=".$aRow['nuptk']."' data-target='#edit_data' class='btn btn-info btn-sm btnku btn-elevate btn-elevate-air' id='edit-data' data-id='".$aRow['nuptk']."'><i class='fa fa-pencil-alt'></i> Edit Data</a>
						...
						$row[] = 
						"<div class='btn-group-vertical' role='group'>
						<a data-toggle='modal' href='guru/form_hapusgurumapel?nuptk=".$aRow['nuptk']."&npsn_nss=".$aRow['npsn_nss']."' data-target='#hapusguru_sekolahmapel' class='btn btn-dark btn-sm btnku btn-elevate btn-elevate-air' id='hapusguru-sekolahmapel' data-id='".$aRow['nuptk']."'><i class='fa fa-eraser'></i> Hapus Sebagai&nbsp;<br/>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;".$aRow['jenis_guru']."&nbsp;</a>
						<a data-toggle='modal' href='guru/form_editguru?nuptk=".$aRow['nuptk']."' data-target='#edit_data' class='btn btn-info btn-sm btnku btn-elevate btn-elevate-air' id='edit-data' data-id='".$aRow['nuptk']."'><i class='fa fa-pencil-alt'></i> Edit Data</a>
						<a data-toggle='modal' href='guru/form_hapusgurusekolah?nuptk=".$aRow['nuptk']."&npsn_nss=".$aRow['npsn_nss']."' data-target='#hapusguru_sekolah' class='btn btn-danger btn-sm btnku btn-elevate btn-elevate-air' id='hapusguru-sekolah' data-id='".$aRow['nuptk']."'><i class='fa fa-eraser'></i> Hapus Sekolah</a>						
						<a data-toggle='modal'  href='guru/form_hapusguru?nuptk=".$aRow['nuptk']."' class='btn btn-sm btn-danger btnku btn-elevate btn-elevate-air' data-target='#hapus_data'  id='hapus-data' data-id='".$aRow['nuptk']."'><i class='fa fa-eraser'></i> Hapus Data</a>
						<a data-toggle='modal'  href='guru/form_gantipasswordguru?nuptk=".$aRow['nuptk']."' class='btn btn-sm btn-dark btnku btn-elevate btn-elevate-air' data-target='#ganti_password'  id='ganti-password' data-id='".$aRow['nuptk']."'><i class='fa fa-lock'></i> Ganti Password</a>
