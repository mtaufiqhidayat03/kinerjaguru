...
class M_sekolahuser extends CI_Model {
	...
	function datagurusdmengajar($search, $page, $id_kelompok){
		...
		$pageku = $page - 1;
		$page2 = $page*10;
		$sQuery = "SELECT b.nuptk, b.nama_guru FROM `".D_GURU_SD.$this->session->userdata('tahun')."` as a left join `".M_GURU_SD."` as b ON b.nuptk=a.nuptk_guru_sd and jenis_guru='".$jenis_guru."'".$tambahan." where nama_guru like '%".$search."%' or nuptk like '%".$search."%' LIMIT ".$pageku.",".$page2;
		} else if ($page > 1) {
		$pageku = ($page - 1) * 10;
		$page2 = $page*10;
		$sQuery = "SELECT b.nuptk, b.nama_guru FROM `".D_GURU_SD.$this->session->userdata('tahun')."` as a left join `".M_GURU_SD."` as b ON b.nuptk=a.nuptk_guru_sd and jenis_guru='".$jenis_guru."'".$tambahan." where nama_guru like '%".$search."%' or nuptk like '%".$search."%' LIMIT ".$pageku.",".$page2;
    ...
    function updatesekolah($data_sekolah,$npsn_nss) {
        ...
        $this->db->update(M_SD, $data_sekolah);
	}
	
    function updatekepalasekolah($data_kepala) {
		$this->db->insert("`".D_SD.$this->session->userdata('tahun')."`", $data_kepala);
	}
	...
	function deletekepalasekolah($npsn_nss){
        $this->db->where('npsn_nss_sd', $npsn_nss);
	...
	$sTable = "`".M_SD."` as a left join master_daerah as b ON a.no_daerah=b.no_daerah left join `".D_SD.$_SESSION["tahun"]."`  as a1 ON a.npsn_nss=a1.npsn_nss_sd left join `".M_GURU_SD."` as c ON a1.nip_kepala=c.nip";
	/*  Paging */
	$sLimit = "";
