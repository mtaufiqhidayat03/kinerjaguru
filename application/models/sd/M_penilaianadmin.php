...
class M_penilaianadmin extends CI_Model {
	...
	function getdatakinerja3($id_indikator, $id_kompetensi, $nuptk, $id_penilaian) {
		$sql="SELECT * FROM `".M_INDIKATOR_SD."` as a left join `".M_KOMPETENSI_SD."` as b ON a.id_kompetensi_indikator_sd=b.id_kompetensi left join `".D_PENILAIAN_SD.$this->session->userdata("tahun")."` as d ON a.id_indikator=d.id_indikator_penilaian_sd where a.id_indikator=? and b.id_kompetensi=? and d.nuptk_penilaian_sd=? and d.id_penilaian=?";
	...
	function deletehasilpenilaian($id_indikator, $nuptk_guru_sd, $id_penilaian){
	...
	function penilaian_list() {
		...
		$params = $_REQUEST;
		$aColumns = array('id_penilaian','id_penilaian','nuptk_penilaian_sd','nama_guru','if(kelompok_kompetensi IS NOT NULL, kelompok_kompetensi,"<span class=\"kt-badge kt-badge--inline kt-badge--danger\"><i class=\"flaticon2-delete\"></i>Guru pindah mengajar kelas/mata pelajaran lain</span>")','nama_kompetensi','nama_indikator', 'if(id_penilaian IS NOT NULL,"<span class=\"kt-badge kt-badge--inline kt-badge--success\"><i class=\"flaticon2-checkmark\"></i>Sudah</span>","<span class=\"kt-badge kt-badge--inline kt-badge--danger\"><i class=\"flaticon2-delete\"></i>Belum</span>")','skor','if(skor IS NOT NULL,"<span class=\"kt-badge kt-badge--inline kt-badge--success\"><i class=\"flaticon2-checkmark\"></i>Sudah</span>","<span class=\"kt-badge kt-badge--inline kt-badge--danger\"><i class=\"flaticon2-delete\"></i>Belum</span>")','id_kelompok','id_indikator_penilaian_sd','id_kompetensi','no_urut_kompetensi','no_urut_indikator','id_indikator','kelompok_kompetensi');
		$sIndexColumn = "e.id_penilaian";
		$sTable = "`".D_PENILAIAN_SD.$this->session->userdata("tahun")."` as e left join `".M_INDIKATOR_SD."` as a ON a.id_indikator=e.id_indikator_penilaian_sd left join `".D_GURU_SD.$this->session->userdata("tahun")."` as d ON d.nuptk_guru_sd=e.nuptk_penilaian_sd left join `".M_KOMPETENSI_SD."` as b ON a.id_kompetensi_indikator_sd=b.id_kompetensi and a.keaktifan_indikator='Aktif' and b.keaktifan='Aktif' left join `".M_KELOMPOK_KOMPETENSI_SD."` as c ON b.id_kelompok_kompetensi_sd=c.id_kelompok and c.hub_jenis_guru=d.jenis_guru and FIND_IN_SET(d.detail_guru,c.hub_detail_guru) left join `".M_GURU_SD."` as f ON e.nuptk_penilaian_sd=f.nuptk ";
		$sLimit = "";
		/*  Paging */
