...
class Sekolah extends CI_Controller {
    function __construct() {
        parent::__construct();
        $this->load->model(FOLDER_SMP_USER.'m_sekolahuser');        
    }
    ...
    function index() {
        if ( $this->session->userdata('status') != "login" and empty($this->session->userdata('username')) and empty($this->session->userdata('id_user')))
        {
            ...
            $this->load->view(FOLDER_SMP_USER.'datasekolah' );
        }
    }
	...
	function ambildatagurusmp(){
		...
		$data = $this->m_sekolahuser->datagurusmp($search, $page);
		$key=0;
		$list = array();
		foreach ($data as $row)
		{
		...
		$page2 = $page * 10;
		if ($search !== "" ) {
			$sQuery = "SELECT COUNT(".$sIndexColumn.") as 'Count' FROM `".M_GURU_SMP."` where nip like '%".$search."%' or nama_guru like '%".$search."%'";
		} else {
            $sQuery = "SELECT COUNT(".$sIndexColumn.") as 'Count' FROM `".M_GURU_SMP."`";
        }
	...
	function ambildatagurusmpmengajar(){
		...
		$data = $this->m_sekolahuser->datagurusmpmengajar($search, $page, $id_kelompok);
		$key=0;
		$list = array();
		foreach ($data as $row)
		{
    ...
    function form_addsekolah(){
        ...
        if ( $this->session->userdata('status') == "login" and !empty($this->session->userdata('username')) and !empty($this->session->userdata('id_user')))
        {
            ...
            $this->load->VIEW(FOLDER_SMP_USER.'form_addsekolah', $data);
        } else {
            $this->load->VIEW(FOLDER_SMP_USER.'v_sesiberakhir');
        }
        } else {
            show_404();
        }
    ...
    function form_hapussekolah() {
          if (isset( $_SERVER['HTTP_X_REQUESTED_WITH'] ) && ($_SERVER['HTTP_X_REQUESTED_WITH'] == 'XMLHttpRequest')) {
            ...
            $this->load->VIEW(FOLDER_SMP_USER.'form_hapussekolah', $data);
        } else {
            show_404();
        }
    }
    ...
    function form_editsekolah() {
          if (isset( $_SERVER['HTTP_X_REQUESTED_WITH'] ) && ($_SERVER['HTTP_X_REQUESTED_WITH'] == 'XMLHttpRequest')) {
            ...
            $this->load->view(FOLDER_SMP_USER.'form_editsekolah', $data);
        } else {
            show_404();
        }
	}
    ...
    function form_kepalasekolah(){
        if (isset( $_SERVER['HTTP_X_REQUESTED_WITH'] ) && ($_SERVER['HTTP_X_REQUESTED_WITH'] == 'XMLHttpRequest')) {
			...
			$this->load->view(FOLDER_SMP_USER.'form_kepalasekolah', $data);
	...
	function form_hapuskepalasekolah() {
		if (isset( $_SERVER['HTTP_X_REQUESTED_WITH'] ) && ($_SERVER['HTTP_X_REQUESTED_WITH'] == 'XMLHttpRequest')) {
		  $npsn_nss = $this->input->get('npsn_nss');
		  $data['n1'] = $this->m_sekolahuser->getdatasekolah($npsn_nss);
		  $this->load->view(FOLDER_SMP_USER.'form_hapuskepalasekolah', $data);
	  } else {
		  show_404();
	  }
  }
