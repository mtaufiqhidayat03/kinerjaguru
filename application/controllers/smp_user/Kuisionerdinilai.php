...
class Kuisionerdinilai extends CI_Controller {
    function __construct() {
        parent::__construct();
		$this->load->model(FOLDER_SMP_USER.'m_kuisionerdinilai');   
    ...
    function index() {
        if ( $this->session->userdata('status') != "login" and empty($this->session->userdata('username')) and empty($this->session->userdata('id_user')))
        {
            ...
            if (isset($npsn_nss) and $npsn_nss !== "") { 
			  ...
			  $this->load->view(FOLDER_SMP_USER.'datakuisionerdinilai', $data);  
            } else {
              $data['n20'] = ""; 
			  $this->load->view(FOLDER_SMP_USER.'datakuisionerdinilai');
            }
        }
	}
    ...
    function form_addassesor(){
        ...
        if ( $this->session->userdata('status') == "login" and !empty($this->session->userdata('username')) and !empty($this->session->userdata('id_user')))
        { 
            $this->load->view(FOLDER_SMP_USER.'form_addassesor');
        } else {
	...
	function form_lihatpdfkuisionerdinilai() {
		...
		if ( $this->session->userdata('status') == "login" and !empty($this->session->userdata('username')) and !empty($this->session->userdata('id_user')))
        {
			...
			$no_kuisioner = $this->input->get('no_kuisioner');
		  	...
		  	$this->load->view(FOLDER_SMP_USER.'form_lihatpdfkuisionerdinilai', $data);
		} else {
            $this->load->view('v_sesiberakhir');
        }
	  	} else {
	  ...
	  function form_persetujuankuisioner() {
		...
		if ( $this->session->userdata('status') == "login" and !empty($this->session->userdata('username')) and !empty($this->session->userdata('id_user')))
        {
		  ...
		  $this->load->view(FOLDER_SMP_USER.'form_persetujuankuisioner', $data);
		} else {
            $this->load->view('v_sesiberakhir');
        }
	  	} else {
	  ...
	  function aksipersetujuankuisioner() {
		...
		if ( $this->session->userdata('status') == "login" and !empty($this->session->userdata('username')) and !empty($this->session->userdata('id_user')))
        {
			...
			$data = $this->m_kuisionerdinilai->editpenilaiankuisioner2($data_kuisioner, $no_kuisioner, $id_kuisioner, $nuptk_guru_smp);
			if ($this->db->affected_rows() != 1) {
