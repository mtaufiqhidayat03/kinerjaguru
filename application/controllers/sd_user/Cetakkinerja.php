...
class Cetakkinerja extends CI_Controller {
	...
	function lampiransatud() {
		...
		foreach ($data10 as $row10) {
            if (strtolower($row10->nama_kuisioner) != "tidak hadir tanpa keterangan") {
                $no = $no + 1;
		...
		$cetak .=	"<tr valign='middle'>
					...
					<td colspan='3' style='border: 1px solid black;'><center>".$golonganku."</center></td>
					</tr>";
		$cetak .= "</table>";	
		$cetak .= "<br/>";
		$cetak .= "<table width=100% class='center'>";
	...
	function cetakpenilaian(){
		...
		.hangingindent {padding-left: 24px; text-indent: -20px;}</style>";
