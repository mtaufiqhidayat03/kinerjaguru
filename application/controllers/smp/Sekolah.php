...
class Sekolah extends CI_Controller {
    function __construct() {
        parent::__construct();
        $this->load->model(FOLDER_SMP.'m_sekolahadmin');        
    }
    ...
    function index() {
        if ( $this->session->userdata('status') != "login" and empty($this->session->userdata('username')) and empty($this->session->userdata('id_user')))
        {
            ...
            if (($this->session->userdata('level') == "Administrator Kota") or ($this->session->userdata('level') == "Administrator Sekolah")) {
                $this->load->view(FOLDER_SMP.'datasekolah');
            } else {
				show_404();
			}
        }
	...
	function ambildatagurusmp(){
		...
		$data = $this->m_sekolahadmin->datagurusmp($search, $page, $npsn_nss);
		$key=0;
		$list = array();
		foreach ($data as $row)
		{
		...
		$page2 = $page * 10;
		if ($search !== "" ) {
			$sQuery = "SELECT COUNT(".$sIndexColumn.") as 'Count' FROM `".M_GURU_SMP."` as a left join `".D_GURU_SMP.$this->session->userdata('tahun')."` as b ON a.nuptk=b.nuptk_guru_smp where npsn_nss_guru_smp='".$npsn_nss."' and (nip like '%".$search."%' or nama_guru like '%".$search."%')";
		} else {
            $sQuery = "SELECT COUNT(".$sIndexColumn.") as 'Count' FROM `".M_GURU_SMP."` as a left join `".D_GURU_SMP.$this->session->userdata('tahun')."` as b ON a.nuptk=b.nuptk_guru_smp where npsn_nss_guru_smp='".$npsn_nss."'";
        }
	...
	function ambildatagurusmpmengajar(){
		if (isset( $_SERVER['HTTP_X_REQUESTED_WITH'] ) && ($_SERVER['HTTP_X_REQUESTED_WITH'] == 'XMLHttpRequest')) 
		{
		if ( $this->session->userdata('status') == "login" and !empty($this->session->userdata('username')) and !empty($this->session->userdata('id_user')))
		{
		...
		$data = $this->m_sekolahadmin->datagurusmpmengajar($search, $page, $id_kelompok);
		$key=0;
		$list = array();
		foreach ($data as $row)
		{
		...
		$page2 = $page * 10;
		if ($search !== "" ) {
			$sQuery = "SELECT COUNT(".$sIndexColumn.") as 'Count' FROM `".D_GURU_SMP.$this->session->userdata('tahun')."` as a left join `".M_GURU_SMP."` as b ON b.nuptk=a.nuptk_guru_smp and jenis_guru='".$jenis_guru."'".$tambahan." where nama_guru like '%".$search."%' or nuptk like '%".$search."%'";
		} else {
            $sQuery = "SELECT COUNT(".$sIndexColumn.") as 'Count' FROM `".D_GURU_SMP.$this->session->userdata('tahun')."` as a left join `".M_GURU_SMP."` as b ON b.nuptk=a.nuptk_guru_smp and jenis_guru='".$jenis_guru."'".$tambahan;
        }
		$rResultTotal = $this->db->query($sQuery);
		$aResultTotal = $rResultTotal->row()->Count;
		$iTotal = $aResultTotal;
    ...
    function aksieditkepalasekolah(){
		if (isset( $_SERVER['HTTP_X_REQUESTED_WITH'] ) && ($_SERVER['HTTP_X_REQUESTED_WITH'] == 'XMLHttpRequest')) {
			...
			$query = $this->db->get_where("`".D_SMP.$this->session->userdata('tahun')."`", array('npsn_nss_smp' => $npsn_nss));
			$nipkepala=$this->input->post('editkepala_sekolah');
			$query2 = $this->db->get_where("`".D_SMP.$this->session->userdata('tahun')."`", array('nip_kepala' => $nipkepala));
				if ($query->num_rows() == 0 && $query2->num_rows() == 0) {
				$data_kepala = array(  
					'npsn_nss_smp'=>$npsn_nss,            
    ...
    function aksiaddsekolah(){
        ...
        {
        $npsn_nss=$this->input->post('npsn_nss');
        $query = $this->db->get_where(M_SMP, array('npsn_nss' => $npsn_nss));
            if ($query->num_rows() == 0) {
            $data_sekolah = array(
                'npsn_nss'=>$this->input->post('npsn_nss'),
    ...
    function form_addsekolah(){
        ...
        if ( $this->session->userdata('status') == "login" and !empty($this->session->userdata('username')) and !empty($this->session->userdata('id_user')))
        {
            ...
            $this->load->VIEW(FOLDER_SMP.'form_addsekolah', $data);
        } else {
            $this->load->VIEW(FOLDER_SMP.'v_sesiberakhir');
        }
        } else {
            show_404();
        }
    ...
    function form_hapussekolah() {
          if (isset( $_SERVER['HTTP_X_REQUESTED_WITH'] ) && ($_SERVER['HTTP_X_REQUESTED_WITH'] == 'XMLHttpRequest')) {
            ...
            $this->load->VIEW(FOLDER_SMP.'form_hapussekolah', $data);
        } else {
            show_404();
        }
    }
    ...
    function form_editsekolah() {
          if (isset( $_SERVER['HTTP_X_REQUESTED_WITH'] ) && ($_SERVER['HTTP_X_REQUESTED_WITH'] == 'XMLHttpRequest')) {
            ...
            $this->load->view(FOLDER_SMP.'form_editsekolah', $data);
        } else {
            show_404();
        }
	}
    ...
    function form_kepalasekolah(){
        if (isset( $_SERVER['HTTP_X_REQUESTED_WITH'] ) && ($_SERVER['HTTP_X_REQUESTED_WITH'] == 'XMLHttpRequest')) {
			...
			$this->load->view(FOLDER_SMP.'form_kepalasekolah', $data);
	...
	function form_hapuskepalasekolah() {
		if (isset( $_SERVER['HTTP_X_REQUESTED_WITH'] ) && ($_SERVER['HTTP_X_REQUESTED_WITH'] == 'XMLHttpRequest')) {
		  $npsn_nss = $this->input->get('npsn_nss');
		  $data['n1'] = $this->m_sekolahadmin->getdatasekolah($npsn_nss);
		  $this->load->view(FOLDER_SMP.'form_hapuskepalasekolah', $data);
	  } else {
		  show_404();
	  }
  }
