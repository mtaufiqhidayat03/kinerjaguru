...
class Assesor extends CI_Controller {
    function __construct() {
		parent::__construct();
            ...
            $this->load->model(FOLDER_SMP.'m_assesoradmin');
    ...
    function index() {
		...
		if (($this->session->userdata('level') == "Administrator Kota") or ($this->session->userdata('level') == "Administrator Sekolah")) {
            ...
            if (isset($npsn_nss) and $npsn_nss !== "") {
                ...
                $this->load->view(FOLDER_SMP.'dataassesor', $data);
            } else {
                $data['n20'] = "";
                $this->load->view(FOLDER_SMP.'dataassesor');
            }
        } else {
			show_404();
		}
    ...
    function form_addassesor(){
        ...
        if ( $this->session->userdata('status') == "login" and !empty($this->session->userdata('username')) and !empty($this->session->userdata('id_user')))
        { 
            $this->load->view(FOLDER_SMP.'form_addassesor');
        } else {
    ...
    function aksiaddassesor(){
        ...
        $query = $this->db->get_where(D_ASSESOR_SMP.$this->session->userdata("tahun"), array('nuptk_assesor' => $nuptk_assesor, 'tugas_assesor' => $tugas_assesor));
            if ($query->num_rows() == 0) {
            $data_assesor = array(
                'nuptk_assesor'=>$this->input->post('assesor'),
                'tugas_assesor'=>$this->input->post('guru_dinilai'),
    ...
    function ambildataassesor(){
		...
		$page2 = $page * 10;
		if ($search !== "" ) {
			$sQuery = "SELECT COUNT(".$sIndexColumn.") as 'Count' FROM `".D_GURU_SMP.$this->session->userdata('tahun')."` as a left join `".M_GURU_SMP."` as b ON a.nuptk_guru_smp=b.nuptk where nuptk like '%".$search."%' or nama_guru like '%".$search."%'";
		} else {
            $sQuery = "SELECT COUNT(".$sIndexColumn.") as 'Count' FROM `".D_GURU_SMP.$this->session->userdata('tahun')."` as a left join `".M_GURU_SMP."` as b ON a.nuptk_guru_smp=b.nuptk";
        }
	...
	function ambildatagurudinilai(){
		...
		$page2 = $page * 10;
		if ($search !== "" ) {
			$sQuery = "SELECT COUNT(".$sIndexColumn.") as 'Count' FROM `".D_GURU_SMP.$this->session->userdata('tahun')."` as a left join `".M_GURU_SMP."` as b ON a.nuptk_guru_smp=b.nuptk where npsn_nss_guru_smp='".$npsn_nss_assesor."' and nuptk_guru_smp !='".$assesor."' and npsn_nss_guru_smp ='".$npsn_nss_assesor."'  and (nuptk like '%".$search."%' or nama_guru like '%".$search."%')";
		} else {
            $sQuery = "SELECT COUNT(".$sIndexColumn.") as 'Count' FROM `".D_GURU_SMP.$this->session->userdata('tahun')."` as a left join `".M_GURU_SMP."` as b ON a.nuptk_guru_smp=b.nuptk where npsn_nss_guru_smp = '".$npsn_nss_assesor."' and nuptk_guru_smp !='".$assesor."' and npsn_nss_guru_smp ='".$npsn_nss_assesor."'";
        }
		$rResultTotal = $this->db->query($sQuery);
		$aResultTotal = $rResultTotal->row()->Count;
		$iTotal = $aResultTotal;
    ...
    function form_hapusassesor() {
          if (isset( $_SERVER['HTTP_X_REQUESTED_WITH'] ) && ($_SERVER['HTTP_X_REQUESTED_WITH'] == 'XMLHttpRequest')) {
            ...
            $this->load->view(FOLDER_SMP.'form_hapusassesor', $data);
        } else {
            show_404();
        }
    }
